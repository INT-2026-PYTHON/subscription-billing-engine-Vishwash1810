...
class PercentageDiscount(Discount):
    def __init__(self, percentage: Decimal) -> None:
        if isinstance(percentage, float):
            raise TypeError("PercentageDiscount rate must not be float")
        if not isinstance(percentage, Decimal):
            raise TypeError("PercentageDiscount rate must be Decimal")
        if percentage < 0 or percentage > 1:
            raise ValueError("PercentageDiscount rate must be between 0 and 1")
